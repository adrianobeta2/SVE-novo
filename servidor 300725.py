...
def initialize_camera_basler():
    """ Inicializa o watchdog da câmera Basler em uma thread dedicada """
    threading.Thread(target=camera_watchdog_loop, daemon=True).start()

def camera_watchdog_loop():
    """ Loop de watchdog que mantém a câmera Basler ativa continuamente """
    global camera, capture_thread_running
    ...
    while True:
        try:
            ...
            if camera is None:
                print("[WATCHDOG] Câmera não encontrada. Aguardando reconexão...")
                time.sleep(3)
                continue  # Tenta de novo

            print("[WATCHDOG] Câmera encontrada. Iniciando configuração...")
            camera.Open()
            ...
            capture_thread_running = True
            print("[WATCHDOG] Iniciando captura de frames...")
            camera_loop_basler(converter)

        except Exception as e:
            print(f"[WATCHDOG] Erro na câmera: {e}")
            print("[WATCHDOG] Tentando reconectar em 3 segundos...")
            ...
            try:
                ...
                print(f"[WATCHDOG] Erro ao fechar câmera: {e_close}")

            time.sleep(3)
            # Recomeça o loop e tenta novamente

def camera_loop_basler(converter):
    """ Captura frames continuamente e redimensiona antes de armazenar """
    global last_frame, capture_thread_running, camera
    ...
    while capture_thread_running and camera.IsGrabbing():
        try:
            ...
            print(f"[CAPTURA] Erro ao capturar frame: {e}")
            capture_thread_running = False
            break  # Sai do loop de captura; watchdog irá reiniciar
        ...
        for i, (x, y, w, h,textura_tolerancia,pixel_tolerancia, threshold_cor,ponto_tolerancia) in enumerate(rois, start=1):
            ...
            if textura == True:
                ...
                if(status_texutura==False):
                   status_texutura, media_textura = AnaliseTextura.textura_analise_NOK(roi,programa,gray,mask,coordenadas,textura_tolerancia,contrast, brightness, gamma, exposure)
